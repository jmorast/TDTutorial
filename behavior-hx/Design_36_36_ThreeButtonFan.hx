...
class Design_36_36_ThreeButtonFan extends ActorScript
{          	
...
propertyChanged("_ButtonsVisable", _ButtonsVisable);
        ...
        _b1.moveBy(-50, -35, _AnimationDuragion, Quad.easeIn);
        _b2.enableActorDrawing();
        _b2.moveBy(0, -50, _AnimationDuragion, Quad.easeIn);
        _b3.enableActorDrawing();
        _b3.moveBy(50, -35, _AnimationDuragion, Quad.easeIn);
}
...
{
            actor.say("Three Button Fan", "_customBlock_show_buttons");
}

        else
{
            actor.say("Three Button Fan", "_customBlock_hide_buttons");
            if(_b1.isMouseDown())
{
                runLater(1000 * _AnimationDuragion, function(timeTask:TimedTask):Void {
